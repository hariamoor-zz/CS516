...
// Go through XML format KDG build data structure
...
string Parser::genObjectiveFunction(){
    ...
    cout << "Knobs are:" << endl;
    // cout << *(graph_->getKnobs()) << endl;
    // cout << (graph_->getKnobs()->empty()) ? "Empty" : "Not empty" << endl;
    if (graph_->getKnobs()->empty()) {
        cout << "Empty" << endl;
    } else {
        cout << "Not empty" << endl;
    }

    for (Knob *knob : *(graph_->getKnobs())) {
        cout << "something" << endl;
        for (Level *level : *(knob->getLevelNodes())) {
            for (Basic *basic : *(level->getBasicNodes())) {
                // stringstream magic
                string term = (first) ? basic->getName() : " +" + basic->getName();
